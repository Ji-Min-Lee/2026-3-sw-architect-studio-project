...
| **2nd** | **지원 BPH 범위 확대** | 정확도를 유지하면서 더 높은 BPH(빠른 시계)까지 커버하는 것이 두 번째 목표 |
...
| FR-09 | Pause + timeline navigation | LOW | ❌ Not implemented |
...
> ⚠️ **잠정값**: 목표 SPS 및 Dropped Block 달성 여부는 **EX-01** 실측 전까지 확정 불가.
...
| **Environment** | Raspberry Pi 5 (8GB RAM), Ubuntu 24.04, Live 모드 실행 중, 28,800 BPH 기계식 시계, 연속 10분 이상, Qt GUI + DSP 동시 실행<br>• **Minimum**: 48,000 sps (이 이하면 프로젝트 실패)<br>• **Objective**: 96,000 sps *(잠정, EX-01 확정)*<br>• **Stretch**: 192,000 sps |
...
| 실험으로 검증 가능한가? | ✅ EX-01: 48k/96k/192k sps × 10분 연속 실행 |
...
| **Environment** | Raspberry Pi 5 (8GB RAM), Ubuntu 24.04, Live mode, 28,800 BPH mechanical watch, continuous ≥ 10 min, Qt GUI + DSP running concurrently<br>• **Minimum**: 48,000 sps (below this = project failure)<br>• **Objective**: 96,000 sps *(provisional, confirmed by EX-01)*<br>• **Stretch**: 192,000 sps |
...
> ⚠️ **잠정값**: 아래 수치는 **EX-L1** 실측 전까지 확정 불가.
...
> **Stretch (Option B)**: EX-L1 결과 후 36,000 / 43,200 BPH까지 상향 검토
...
| 실험으로 검증 가능한가? | ✅ EX-L1: 3구간 × SPS 3단계 × 탭 구성 2가지 × 10분 |
...
| **Response Measure** | • **QA-C1**: 동일 beat 데이터에서 파생된 모든 뷰의 수치 편차: **0** — Observer 패턴으로 구조적 보장<br>• **QA-C2**: 소음 3조건에서 Δ Rate / Δ Amplitude / Δ Beat Error를 최소화하는 Detector 파라미터(`onset_fraction`, `min_peak_fraction`) 값 — **EX-04 결과 후 확정** |
...
| pass/fail 기준이 명확한가? | ✅ Observer 패턴 적용 시 자동 PASS | ⚠️ 허용 Δ 수치는 EX-04 후 확정 |
| 실험으로 검증 가능한가? | ✅ 구조적 보장 (실험 불필요) | ✅ EX-04: 소음 3조건 × Detector 파라미터 조합 |
...
| **Response Measure** | • **QA-C1**: Value deviation across all views derived from same beat data: **0** — structurally guaranteed by Observer pattern<br>• **QA-C2**: Optimal Detector parameters (`onset_fraction`, `min_peak_fraction`) minimizing Δ across 3 noise conditions — **finalized after EX-04** |
...
| Detector 파라미터 튜닝 (EX-04) | QA-C2 | onset_fraction / min_peak_fraction 최적값 실험으로 확정 |
...
|----|:------:|---------|----------|
| Real-Time Performance | 1 | Lock-Free Ring Buffer, Priority Scheduling, Graceful Degradation | EX-01 (SPS별 Dropped Block) |
| Low Latency | 2 | Thread 분리 (Concurrency), Lock-Free Ring Buffer, Lazy Rendering | EX-L1 (3구간 타임스탬프 측정) |
| Correctness | 3 | Observer/Signal-Slot, Adaptive threshold, Detector 파라미터 튜닝 | EX-04 (소음 조건 × 파라미터 Δ) |
...
|----|:-------:|---------------|----------------|
| Real-Time Performance | 1 | Lock-Free Ring Buffer, Priority Scheduling, Graceful Degradation | EX-01 (Dropped Block per SPS) |
| Low Latency | 2 | Thread separation (Concurrency), Lock-Free Ring Buffer, Lazy Rendering | EX-L1 (3-segment timestamp measurement) |
| Correctness | 3 | Observer/Signal-Slot, Adaptive threshold, Detector parameter tuning | EX-04 (noise × parameter Δ) |
...
| 제약 | 내용 | 변경 불가 이유 |
|------|------|-------------|
| 타겟 하드웨어 | Raspberry Pi 5 (ARM64, 8GB RAM) + USB 오디오 센서 + 8" 터치스크린 | 프로그램에서 하드웨어 지급 |
...
|------------|--------|-----------|
| Target Hardware | Raspberry Pi 5 (ARM64, 8GB RAM) + USB audio sensor + 8" touchscreen | Hardware provided by program |
...
| ID | 질문 | 해결 방법 | 영향 QA |
|----|------|---------|--------|
| OI-P1 | RPi 5에서 96k sps Dropped Block = 0 달성 가능한가? | EX-01 | QAS-1 |
| OI-L1 | QAudioSource 라이브 캡처의 실제 콜백 주기는? | EX-L1 실측 | QAS-2 |
| OI-L2 | 11탭 동시 렌더링 시 ② process→display < 30ms인가? | EX-L1 실측 | QAS-2 |
| OI-L3 | 28,800 BPH 달성 후 36,000 / 43,200 BPH까지 상향 가능한가? | EX-L1 결과 후 팀 합의 | QAS-2 |
| OI-C1 | 소음 3조건에서 Δ를 최소화하는 Detector 파라미터 최적값은? | EX-04 | QAS-3 |
...
|----|--------------|-----------|------------|
| OI-P1 | Can RPi 5 achieve Dropped Block = 0 at 96k sps? | EX-01 | QAS-1 |
| OI-L1 | What is the actual QAudioSource live callback period? | EX-L1 measurement | QAS-2 |
| OI-L2 | Is ② process→display < 30ms with 11 tabs rendering? | EX-L1 measurement | QAS-2 |
| OI-L3 | After achieving 28,800 BPH target, can the target be raised to 36,000 / 43,200 BPH? | EX-L1 result → team decision | QAS-2 |
| OI-C1 | What Detector parameter values minimize Δ across 3 noise conditions? | EX-04 | QAS-3 |
