...
**Data Sources** (source files referenced in this document):
...
> ⚠️ **Provisional**: Target SPS and Dropped Block = 0 achievement cannot be confirmed until **EXP-01** measurements are conducted.
...
> Higher SPS shortens the block period, reducing the time budget for DSP. SPS is the **cause** (environment condition) of Dropped Blocks; Ring Buffer overflow is the **measurement point**.
...
| Criterion | Result |
|------|------|
| Is it measurable? | ✅ Dropped Block count — directly measurable |
| Is the pass/fail criterion clear? | ✅ Dropped Block = 0 means PASS |
...
**Project Goal Connection:** Dropped Block → T1/T3 timestamp lost → Rate / Amplitude / Beat Error computation fails → team primary goal collapses. **Prerequisite for all other QAs.**
...
| LT-02 | Lock-Free Ring Buffer | Eliminates mutex between threads → prevents latency spikes in segment ① |
| LT-03 | Reduce Computational Overhead | Remove unnecessary computation from the real-time path |
| LT-04 | Lazy Rendering (active tab only) | Reduces Qt main thread rendering load → reduces latency in segment ② |
...
> Detailed QA–tactic–experiment mapping: see **[05-architectural-approaches.md §4 Driver–Approach Traceability](05-architectural-approaches.md#4-drivernapproach-traceability)**.
...
> Full open issues list (project impact, linked risks, actions): see **[03-risk-assessment.md §4 Open Issues](03-risk-assessment.md#4-open-issues--project-outcome-linkage)**.
