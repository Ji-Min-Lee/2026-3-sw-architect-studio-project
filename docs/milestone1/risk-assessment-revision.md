...
| TR-03 | RPi에서 48k/96k/192k sps + Qt GUI 동시 처리 시 CPU 과부하 및 end-to-end 레이턴시 초과 가능성 미검증 / RPi CPU overload and end-to-end latency violation risk at 48k/96k/192k sps with Qt GUI unverified | H | H | OI-03 |
...
| OI-03 | GUI 실행 중 RPi 최대 성능 미측정 (sps별 CPU 부하, Dropped block, FPS, end-to-end 레이턴시 3구간) / RPi performance under load unverified (CPU load per sps, dropped blocks, FPS, 3-segment end-to-end latency) | TR-03, TR-04, TR-07 | 실시간 처리 불가 — 데모에서 오디오 드롭·레이턴시 초과·렌더링 실패 / Real-time processing fails at demo — audio drops, latency overrun, rendering failure |
...
| OI-01 | **EX-03**: `tg_c_placement_t` 설정별 오차 비교 실험 — `TG_C_PLACEMENT_PEAK` / `TG_C_PLACEMENT_ONSET` 각각으로 동일 시계 측정 후 WeiShi 1000 기준 Rate/Amplitude 오차 비교 (Beat Error는 A 이벤트만으로 계산되므로 placement 무관, 측정 대상에서 제외; onset/peak 감지 자체는 코드에 구현 완료, 최적 설정 선택이 목적) / Compare error by `tg_c_placement_t` setting — measure same watch with PEAK vs ONSET placement, compare Rate/Amplitude vs WeiShi 1000 (Beat Error excluded — computed from A events only, unaffected by placement; detection already implemented; goal is selecting optimal setting) | placement 설정 결정 + Rate/Amplitude 오차 margin 수치 확보 / Placement setting decided + Rate/Amplitude error margin measured |
| OI-03 | **EX-01**: RPi에서 48k / 96k / 192k sps × Qt GUI 조건별 CPU 점유율 · Dropped block · GUI FPS · OS 오디오 콜백 주기 · end-to-end 레이턴시 3구간(① capture→process / ② process→display / ③ end-to-end) 동시 실측. sps 선택 기준 = CPU 부하 (Dropped block = 0 유지 가능한 최고 sps); capture→process 하한은 sps와 무관하게 OS 콜백 주기로 결정됨을 실측 확인 / EX-01: Measure CPU usage, dropped blocks, GUI FPS, OS callback period, and 3-segment latency (① capture→process / ② process→display / ③ end-to-end) at 48k/96k/192k sps with Qt GUI. sps criterion = CPU load (max sps with Dropped block = 0); confirm capture→process floor is set by OS callback period, not sps. | sps별 CPU 성능 + 3구간 레이턴시 실측 수치 확보; OS 콜백 주기 확인; QAS-1 목표 sps 및 QAS-3 레이턴시 잠정값 확정; 48k 폴백 여부 결정 완료 / Per-sps CPU data + 3-segment latency figures collected; OS callback period confirmed; QAS-1 sps and QAS-3 latency targets finalized; 48k fallback decided |
| OI-04 | `AudioCapture` / `MeasurementEngine` 분리 범위 확정 후 4-layer 모듈 경계 문서화 / Confirm split scope and document 4-layer boundaries | Project Plan 작성 시 담당자 및 분리 범위 확정 후 모듈 경계 문서화 완료 / Ownership and split scope confirmed in Project Plan; 4-layer boundaries documented |
| OI-05 | macOS 오디오 실제 동작을 목표로 `Q_OS_MAC` 분기 추가 및 CoreAudio / PortAudio 연동 구현 / Add `Q_OS_MAC` branch and implement CoreAudio / PortAudio integration for real audio on macOS | macOS에서 빌드 성공 + 오디오 실제 동작 확인 (Sim 모드 포함) / Build succeeds on macOS and audio confirmed working (including Sim mode) |
