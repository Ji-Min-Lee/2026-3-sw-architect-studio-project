...
> | **Option B** *(현재 잠정)* | ~43,200 BPH (worst-case) | **< 66 ms** | **< 46 ms** | **< 20 ms** |
>
> *현재 잠정값은 Option B (worst-case 43,200 BPH 기준). EX-01 결과 및 보유 시계 BPH 확인 후 팀 합의로 확정.*

**공통 제약**: Dropped audio block: 0, Missed beat: 0  
*([EX-01](./planned-experiments.md#ex-01-rpi-성능-벤치마크--rpi-performance-benchmark) 결과 후 확정)*
...
| **Response Measure** | 새 그래프 1개 추가 시 변경 파일 수: **≤ 3개** (근거 아래 참조)<br>기존 계산 로직 수정 없이 표시 레이어만 추가 가능 |

#### 수치 근거 / "≤ 3 Files" Derivation

**≤ 3개는 이상적인 플러그인 구조에서 이론적 최솟값이다.**

| 변경 유형 / Change Type | 대상 / Target | 파일 수 |
|------------------------|--------------|:------:|
| ① 새 그래프 위젯 파일 신규 생성 / New graph widget file | `GraphFoo.cpp` / `GraphFoo.h` | 1 |
| ② 탭 패널에 새 탭 등록 / Register tab in panel | `TabPanel.cpp` (기존 수정) | 1 |
| ③ 데이터 구독 연결 / Wire data subscription | `DataBroker.cpp` (기존 수정) | 1 |
| **합계 / Total** | | **3** |
