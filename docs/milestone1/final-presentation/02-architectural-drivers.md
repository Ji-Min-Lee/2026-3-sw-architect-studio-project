...
|:----:|----|----------------| :------------------:| :-------------------------:|-------------------|
| **1** | Real-Time Performance | The system shall acquire, process, analyze, and display watch acoustic data in real time while maintaining a responsive GUI | H | H | Prerequisite for all other QAs — Dropped Block destroys T1/T3 timestamps, making measurement impossible |
| **2** | Low Latency | The system shall minimize end-to-end latency between acoustic capture at the microphone and presentation of the corresponding waveform, markers, and computed values in the GUI | H | H | Hard threshold defined by beat period; exceeding it causes functional failure; unverified on RPi + Qt |
