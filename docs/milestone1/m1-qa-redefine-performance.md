...
| **Response** | 각 오디오 블록을 블록 주기 이내에 처리하여 Ring Buffer 오버플로를 방지한다 (Dropped Block = 0). GUI 표시 지연은 QAS-3 범위 / Process each audio block within the block period to prevent Ring Buffer overflow (Dropped Block = 0). GUI display latency is QAS-3 scope |
