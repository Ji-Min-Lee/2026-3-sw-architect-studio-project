...
4개 모듈 계층과 2개 실행 스레드의 매핑을 나타낸다.

```mermaid
flowchart TB
    subgraph AudioThread["Audio Thread (백그라운드 · TimeCriticalPriority)"]
        L1["Acquisition Layer\nLiveCapture / PlaybackCapture / SimCapture\n→ TMasterAudioDataRaw (링버퍼) 에 쓰기"]
    end

    subgraph MainThread["UI Thread (메인 Qt Event Loop)"]
        L2["Signal Processing Layer\nFilterChain (HPF + Envelope)\nBeat Event Detection (Detector + BPH)"]
        L3["Domain Layer\nMeasurementEngine\nMeasurementStore (히스토리 버퍼)"]
        L4["Presentation Layer\nGraphTabManager\n각 그래프 탭 위젯"]
        L2 -->|"beat events\n(T1/T3 타임스탬프)"| L3
        L3 -->|"emit measurementUpdated()\n(Qt Signal)"| L4
    end

    L1 -->|"emit AudioDataReady()\n(Qt Signal-Slot · 스레드 안전)"| L2
```

> **현재 코드와의 차이**: 현재 베이스코드는 L2·L3·L4가 모두 `MainWindow` 하나에 집중된 God Object 구조다.
> 이 아키텍처는 M2 구현에서 `MeasurementEngine`, `MeasurementStore`, `AudioCapture` 추상화로 분리할 계획이다.
...
|---------|---------------------|-------|------|
| QAS-1 실시간 성능 (96k sps, dropped block 0) | AT-01 Pipeline 도입, AT-02 스레드 분리, AT-04 단계적 성능 저하 | ⚠️ 조건부 | 설계 방향은 올바르나 RPi 5에서 96k sps 달성 여부는 EX-01로 검증 필요 |
| QAS-2 측정 정확도 (Rate 오차 < 5 s/d, Beat Error 오차 < 0.1 ms) | AT-01 Pipeline (필터·감지 단계 독립 교체 가능), AT-04 폴백 설계 | ⚠️ 조건부 | Pipeline 단계 분리로 필터·감지 교체 가능. 실제 WeiShi 대비 오차는 EX-02·EX-03으로 확정 |
| QAS-3 저지연 (end-to-end < 100 ms) | AT-01 최소 버퍼 파이프라인 (블록 ≤ 4096 샘플), AT-02 스레드 분리 | ⚠️ 조건부 | 구조적으로 latency 단축 가능하나 100ms 이내 달성 여부는 EX-01로 확정 |
| QAS-4 정합성 (모든 뷰에서 수치 편차 0) | AT-03 4계층 구조 (단일 Domain Layer에서 계산), AP-01 Observer (동일 Signal 구독) | ✅ 설계로 보장 | 단일 `MeasurementEngine`에서 계산 후 동일 Signal을 모든 뷰가 구독 → 뷰 간 편차 = 0이 구조적으로 보장. 실험 불필요 |
...
아키텍처가 포함하지 않는 것 — 설계 논의 대상에서 제외한다.

- 클라우드 연결 또는 원격 데이터 로깅 없음 (온디바이스 전용)
- 커스텀 DSP 하드웨어 없음 (USB 오디오 입력 그대로 사용)
- Witschi Chronoscope UI 완전 복제 아님 (기능적 참고만)
- AI 기능(EX-19)은 선택 사항 — 아키텍처가 이에 의존하면 안 됨
- macOS/Windows 전용 기능 없음 (RPi 5 ARM64가 최종 실행 환경)

---

## 7. 검토 체크리스트

- [ ] 아키텍처 개요 수준의 설명 및 다이어그램 포함 (컨텍스트, 파이프라인, 계층/스레드)
- [ ] 핵심 패턴·전술·설계 전략이 정의됨 (AP-01, AT-01 ~ AT-04)
- [ ] 각 어프로치가 드라이버(QAS-1 ~ QAS-5)와 연결됨
- [ ] 미결 설계 결정이 실험(EX-01 ~ EX-05)과 연결되어 목록화됨
- [ ] M2 구현 가이드로서 충분한 수준 (AT-03 4계층 분리, AP-01 Observer 연결, AT-04 폴백 설계 명시)
