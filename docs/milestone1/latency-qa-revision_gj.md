...
**위치**: `### AT-04` 테이블의 `트레이드오프` 행

| 항목 | 현재 | 수정 |
|------|------|------|
| **트레이드오프** | 낮은 샘플레이트에서 T1/T3 타이밍 해상도 감소 → 측정 정확도(QAS-2)에 영향 가능. **Latency vs 품질 균형 필요.** 정확도 오차 허용 범위는 EX-01 + EX-02 결과 이후 확정 | 낮은 샘플레이트에서 T1/T3 타이밍 해상도 감소 → 측정 정확도(QAS-2)에 영향 가능. **sps 선택 기준은 latency가 아니라 CPU 부하 (Dropped block = 0 유지 가능 여부) vs 신호 해상도 균형.** 정확도 오차 허용 범위는 EX-01 + EX-02 결과 이후 확정 |

**이유**: sps는 latency floor를 직접 결정하지 않으므로 "Latency vs 품질"은 잘못된 트레이드오프 표현이다.
