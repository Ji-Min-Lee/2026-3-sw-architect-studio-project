...
**Measurement Accuracy** is not one QA among equals — it is the criterion the entire architecture is evaluated against. Rate, Amplitude, and Beat Error values must match the WeiShi No.1000 reference device.
...
|:--------:|----|------|-----------|:---------:|
| **Goal** | **Measurement Accuracy** | Why this system exists | Rate / Amplitude / Beat Error must match WeiShi reference. This is the criterion the architecture is evaluated against | 🔶 RPi pending |
| **1** | **Modifiability** | Execution enabler (elevated) | 11 graph tabs must exist before experiments can run on them. Architecture decisions derived from experiments must be applied quickly enough to complete the project on schedule. Without a clean layer structure, both conditions fail | ✅ Verified |
...
| **3** | **Low Latency** | Structural prerequisite | If capture→detect latency exceeds one beat period, T1/T3 event timestamps are wrong. Wrong timestamps corrupt Beat Error and Amplitude calculations | 🔶 macOS ✅, RPi R5 06/23 |
| **4** | **Reliability** | Signal-level prerequisite | LP/HP filtering removes ambient noise and false triggers. Without it, the detector fires on non-beat events — undermining accuracy regardless of pipeline performance | ⏳ EXP-03 06/25 |

**Change from M1**: M1 ranked Modifiability (Extensibility) last (Priority 5, L/L). M2 elevates it to Priority 1 — the 11-graph build plan means the extensible structure must be in place first so experiments can run against real tabs, and so architecture decisions can be applied without structural rework blocking the schedule.

---

## QAS-0: Measurement Accuracy — Governing Goal

> This QA did not exist explicitly in M1. Promoted from "Correctness (QAS-3 M1)" to governing goal in M2 after recognizing that every other QA exists only to make this one achievable.
...
## QAS-4: Modifiability — Priority 4 (Enabler)

> M1 name: "Extensibility (QAS-5)". Renamed to Modifiability in M2 to align with Bass/Clements/Kazman terminology.  
...
|----|----------------------|----------|---------|
| QAS-0 Accuracy | All QAs in combination | All ADRs | EXP-02 macOS validated; RPi R5 pending |
| QAS-1 Real-Time Performance | T2 DSP Offload Thread | [ADR-001](adr/ADR-001-t2-dsp-offload-thread.md) | wait_ms ×32,000 ↓, backlog 0% (EXP-02 R2) |
| QAS-2 Low Latency | QueuedConnection + ring buffer | [ADR-001](adr/ADR-001-t2-dsp-offload-thread.md) | wait_ms 420ms → 0.013ms (EXP-02 R2) |
| QAS-2 (render) | Lazy Rendering isVisible() guard | [ADR-002](adr/ADR-002-r1-lazy-rendering.md) | replot_count ↓75–85% (EXP-02 R3/R4) |
| QAS-3 Reliability | LP/HP FilterChain + Adaptive Threshold | ADR-003 pending EXP-03 | EXP-03 scheduled 06/25 |
| QAS-4 Modifiability | 4-layer structure + IGraphTab | Architecture structure | ≤ 3 file change rule verified on existing tabs |
...
| QAS-3 Correctness | Single QA covering both consistency and noise | Split: false-trigger reliability → QAS-3 (Reliability); value consistency → QAS-0 (Accuracy, governing goal) |
| QAS-5 Extensibility | Provisional (post-refactoring) | Renamed Modifiability; verified by 4-layer + IGraphTab |
| Accuracy | Implicit in Correctness | Promoted to governing goal (QAS-0); drives all other QA priorities |
