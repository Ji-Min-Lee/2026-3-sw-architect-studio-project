...
**QAS-3 scenario**: A developer needs to add a new audio input source (e.g., network stream). In the AS-IS design, this requires modifying `MainWindow` and the now-removed `AudioManager` coordinator — two components unrelated to the new source. In the TO-BE design (P1 + i1 refactors), the developer implements `IAudioSource` only; no other file changes are needed.
