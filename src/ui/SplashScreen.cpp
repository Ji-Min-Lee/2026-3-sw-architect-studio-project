...
QFont SplashScreen::hudFont(int pixelSize, bool bold) const
{
    ...
    if (!f.exactMatch())
        f = QFont(QStringLiteral("Courier New"));
...
void SplashScreen::drawBackdrop(QPainter &p, qreal progress) const
{
    ...
    QRadialGradient halo(width() * 0.5, height() * 0.46, width() * 0.62);
    halo.setColorAt(0.0, withAlpha(kAccentCyan, 22));
    halo.setColorAt(0.35, withAlpha(kAccentViolet, 10));
    ...
    const qreal pulse = 0.5 + 0.5 * std::sin(progress * 18.0);
    p.setPen(QPen(withAlpha(kAccentCyan, int(18 + 24 * pulse)), 1.0));
...
void SplashScreen::drawScopePanel(QPainter &p, const QRectF &card, qreal progress) const
{
    ...
    const QRectF scopeArea(card.left() + 34, card.top() + 52, card.width() - 68, 118);

    const qreal startW = 36.0;
    const qreal startH = 10.0;
    const qreal endW = scopeArea.width();
    const qreal endH = scopeArea.height();

    const qreal panelW = startW + (endW - startW) * eased;
    const qreal panelH = startH + (endH - startH) * eased;
    ...
    if (eased > 0.12) {
        ...
        for (int i = 1; i < rows; ++i) {
            const qreal y = panel.top() + panel.height() * qreal(i) / qreal(rows);
    ...
    if (eased > 0.20) {
        ...
        p.drawText(panel.adjusted(10, 7, -10, -7),
                   Qt::AlignTop | Qt::AlignLeft,
                   QStringLiteral("WATCH AUDIO"));

        const qreal pulse = 0.45 + 0.55 * std::sin(progress * 20.0);
        p.setPen(Qt::NoPen);
        p.setBrush(withAlpha(QColor(74, 222, 128), int(220 * labelAlpha * pulse)));
...
void SplashScreen::drawWaveform(QPainter &p, const QRectF &panel, qreal progress) const
{
    ...
    const int fillAlpha = int(36 * wavePhase);
    QLinearGradient fillGrad(panel.left(), midY, panel.left(), panel.bottom());
    fillGrad.setColorAt(0.0, withAlpha(kAccentCyan, fillAlpha));
    fillGrad.setColorAt(1.0, withAlpha(kAccentCyan, 0));
    p.setPen(Qt::NoPen);
    p.setBrush(fillGrad);
    p.drawPath(fill);

    const int traceAlpha = int(235 * wavePhase);
    QLinearGradient stroke(panel.left(), midY, panel.right(), midY);
    stroke.setColorAt(0.0, withAlpha(QColor(56, 189, 248), traceAlpha));
    stroke.setColorAt(0.45, withAlpha(kAccentCyan, traceAlpha));
    stroke.setColorAt(1.0, withAlpha(QColor(167, 139, 250), traceAlpha));
...
void SplashScreen::drawBranding(QPainter &p, const QRectF &card, qreal progress) const
{
    ...
    const qreal textTop = card.top() + 188;

    p.setPen(withAlpha(kAccentCyan, int(120 * textAlpha)));
    p.setFont(hudFont(9, true));
    p.drawText(QRectF(card.left() + 34, textTop, card.width() - 68, 14),
               Qt::AlignLeft | Qt::AlignVCenter,
               QStringLiteral("MECHANICAL WATCH TIMING ANALYZER"));

    p.setPen(withAlpha(kTextPrimary, alpha));
    QFont title = uiFont(34, QFont::DemiBold);
    title.setLetterSpacing(QFont::AbsoluteSpacing, 4.0);
    p.setFont(title);
    p.drawText(QRectF(card.left() + 34, textTop + 16, card.width() - 68, 42),
               Qt::AlignLeft | Qt::AlignVCenter,
               QStringLiteral("TimeGrapher"));

    const qreal lineY = textTop + 64;
    QLinearGradient accentLine(card.left() + 34, lineY, card.left() + 180, lineY);
    ...
    const qreal logoY = textTop + 70;
    if (!m_lgLogo.isNull()) {
        const int logoH = 20;
        const QPixmap scaled = m_lgLogo.scaledToHeight(logoH, Qt::SmoothTransformation);
        p.drawPixmap(QPointF(card.left() + 34, logoY), scaled);
    } else {
        p.drawText(QRectF(card.left() + 34, logoY, card.width() - 68, 22),
                   Qt::AlignLeft | Qt::AlignVCenter,
                   QStringLiteral("LG Electronics"));
    }

    p.setPen(withAlpha(kTextMuted, int(alpha * 0.95)));
    p.setFont(uiFont(12, QFont::Normal));
    p.drawText(QRectF(card.left() + 34, textTop + 98, card.width() - 68, 20),
               Qt::AlignLeft | Qt::AlignVCenter,
...
void SplashScreen::drawStatusBar(QPainter &p, const QRectF &card, qreal progress) const
{
    ...
    const QRectF metaRow(card.left() + 34, footerTop, card.width() - 68, 16);
    ...
    if (barPhase > 0.05) {
        ...
        const qreal loadT = m_skipRequested ? 1.0 : clamp01(qreal(m_clock.elapsed()) / qreal(kDurationMs));
        ...
        p.setBrush(fill);
        p.drawRoundedRect(QRectF(track.left(), track.top(), track.width() * loadT, track.height()), 1, 1);
    }
